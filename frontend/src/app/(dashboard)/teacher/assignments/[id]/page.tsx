...
export default function GradingPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch submissions and assignment in parallel
        const [subRes, assignRes] = await Promise.all([
          api.get(`/submission/assignment/${params.id}`),
          api.get(`/assignment/${params.id}`)
        ]);
        setSubmissions(subRes.data);
        setAssignmentTitle(assignRes.data?.title || 'Assignment');

        // Pre-fill existing marks & feedback
  ...
  const handleGrade = async (submissionId: string) => {
    ...
    try {
      ...
      setSubmissions(prev => prev.map(s =>
        s.id === submissionId
          ? { ...s, marksAwarded: marks[submissionId], feedback: feedback[submissionId] || '', status: 'Graded' }
          : s
      ));
              ...
              <div key={sub.id} className="glass-card p-6 overflow-hidden">
                ...
                <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-slate-800">Student ID: <span className="font-mono text-xs text-slate-500">{sub.studentId.substring(0, 8)}...</span></p>
                      <p className="text-xs text-slate-400">Submitted: {new Date(sub.submittedAt).toLocaleString()}</p>
                    </div>
                  </div>
                  <span className={sub.status === 'Graded' ? 'badge-teal' : 'badge-info'}>
                    {sub.status}
                  </span>
                </div>
                ...
                <div className="bg-teal-50/50 p-4 rounded-xl border border-teal-100/60">
                  <p className="text-xs font-semibold text-teal-700 uppercase tracking-wider mb-3">
                    {sub.status === 'Graded' ? <span className="flex items-center gap-1.5"><CheckCircle className="w-3.5 h-3.5" /> Graded — Update below</span> : <span className="flex items-center gap-1.5"><FileText className="w-3.5 h-3.5" /> Enter Grade</span>}
                  </p>
